...
const ISSUER = process.env.SSO_ISSUER || 'https://dev.loginproxy.gov.bc.ca/auth/realms/standard'
const JWKSURI =
...
function _verifySecret(currentScopes, tokenString, secret, req, callback, sendError) {
  jwt.verify(tokenString, secret, function(verificationError, decodedToken) {
    ...
    // Get roles from either realm_access.roles or client_roles
    var tokenRoles =
    ...
    if (
      ...
      var roleMatch = currentScopes.some((r) => tokenRoles.indexOf(r) >= 0)
      defaultLog.debug('currentScopes', JSON.stringify(currentScopes))
      defaultLog.debug('token roles', tokenRoles)
...
var hashPassword = function(user, password) {
  if (user.salt && password) {
    var crypto = require('crypto')
    return crypto
      .pbkdf2Sync(password, Buffer.from(user.salt, 'base64'), 10000, 64, 'sha1')
      .toString('base64')
  } else {
    return password
  }
