...
const documentsData = [
  ...
  },
  {
    ...
    documentFileName: 'vanilla.docx',
    tags: [['public']],
    isDeleted: false,
  },
  {
    displayName: 'Confidential File',
    documentFileName: '1099_FBI.docx.gpg',
    tags: [['sysadmin']],
    isDeleted: false,
  },
  {
    displayName: 'Deleted File',
    documentFileName: 'not_petya.exe',
    tags: [['public'], ['sysadmin']],
    isDeleted: true,
  },
]
  ...
  test('publishes a document', (done) => {
    ...
    documentFactory.create('document', existingDocumentData).then((document) => {
      ...
      request(app)
        ...
        .send({})
        .then((response) => {
          Document.findOne({ displayName: 'Existing Document' }).exec(function(
            error,
            updatedDocument,
          ) {
    ...
    setupDocuments(documentsData).then((documents) => {
      Document.findOne({ displayName: 'Vanilla Ice Cream' }).exec(function(error, document) {
        ...
        request(app)
          ...
          .expect(200)
          .then((response) => {
            Document.findOne({ displayName: 'Vanilla Ice Cream' }).exec(function(
              error,
              updatedDocument,
            ) {
