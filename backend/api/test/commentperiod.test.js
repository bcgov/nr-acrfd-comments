...
const commentPeriodsData = [
  {
    ...
    _application: specialApplication._id,
    tags: [['public'], ['sysadmin']],
    isDeleted: false,
  },
  {
    name: 'Vanilla Ice Cream',
    _application: vanillaApplication._id,
    tags: [['public']],
    isDeleted: false,
  },
  {
    name: 'Confidential Comment',
    _application: confidentialApplication._id,
    tags: [['sysadmin']],
    isDeleted: false,
  },
  {
    name: 'Deleted Comment',
    _application: deletedApplication._id,
    tags: [['public'], ['sysadmin']],
    isDeleted: true,
  },
]
    ...
    setupCommentPeriods(commentPeriodsData).then((documents) => {
      request(app)
        ...
        .then((response) => {
          ...
          expect(firstCommentPeriod['tags']).toEqual(
            expect.arrayContaining([['public'], ['sysadmin']]),
          )
    ...
    setupCommentPeriods(commentPeriodsData).then((documents) => {
      CommentPeriod.findOne({ name: 'Vanilla Ice Cream' }).exec(function(error, commentPeriod) {
        ...
        request(app)
          ...
          .expect(200)
          .then((response) => {
            CommentPeriod.findOne({ name: 'Vanilla Ice Cream' }).exec(function(
              error,
              commentPeriod,
            ) {
