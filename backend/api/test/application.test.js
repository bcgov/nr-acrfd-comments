    ...
    setupApplications(applicationsData).then((documents) => {
      request(app)
        ...
        .then((response) => {
          ...
          expect(firstApplication['tags']).toEqual(
    ...
    test('logs in and then retrieves the application with that access token', (done) => {
      request(app)
        ...
        .then((response) => {
          ...
          expect(TTLSUtils.getApplicationByDispositionID).toHaveBeenCalledWith(
            'ACCESS_TOKEN',
            999999,
          )
          done()
        ...
        request(app)
          ...
          .then((response) => {
            ...
            Feature.findOne({ applicationID: response.body['_id'] }).exec(function(error, feature) {
              ...
              expect(featureProperties.TENURE_LEGAL_DESCRIPTION).toEqual(
                'READ THESE BORING LEGAL TERMS.',
              )
