...
exports.publicGet = function (args, res, next) {
  ...
  if (args.swagger.params.appId) {
    ...
    var processedParameters = Utils.getSkipLimitParameters(
    ...
    function (commentPeriodPipeline) {
      Utils.runDataQuery(
        ...
        .then(function (data) {
          ...
          } else if (data && Array.isArray(data) && !args.swagger.params.appId) {
            // For non-appId requests, if no total_items, count the returned items
  ...
  if (args.swagger.params.cpStart && args.swagger.params.cpStart.value !== undefined) {
    ...
    if (queryString.eq) {
      ...
      if (queryString.since) {
        commentPeriodDates.push({
          $gte: ['$commentPeriods.startDate', new Date(queryString.since)],
        })
      }
      if (queryString.until) {
        commentPeriodDates.push({
          $lte: ['$commentPeriods.startDate', new Date(queryString.until)],
        })
      }
    }
  }
