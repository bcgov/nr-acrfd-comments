...
export class ApplicationDetailComponent implements OnInit, OnDestroy {
    ...
    this.route.data
      ...
      .subscribe((data: { application: Application }) => {
        if (data.application) {
          ...
          // Automatically sync with Tantalis on page load. Retired applications are no
          // longer updated in Tantalis so we skip the round-trip for those.
          if (!this.application.meta.isRetired) {
  ...
  private internalDeleteApplication() {
    ...
    if (this.application.meta.currentPeriod) {
      observables = observables.pipe(
        concat(this.commentPeriodService.delete(this.application.meta.currentPeriod)),
      )
    }
    ...
    if (this.application.meta.decision) {
      observables = observables.pipe(
        concat(this.decisionService.delete(this.application.meta.decision)),
      )
    }
    ...
    // delete features
    observables = observables.pipe(
      concat(this.featureService.deleteByApplicationId(this.application._id)),
    )
   ...
   *
   * @param {boolean} [isAutoRefresh=false] When true the refresh was triggered automatically on
   *   page load; the snackbar message will reflect this rather than implying the user acted.
   * @memberof ApplicationDetailComponent
   */
  public refreshApplication(isAutoRefresh = false) {
    ...
    this.api
      ...
      .pipe(
        ...
        mergeMap((updatedApplicationAndFeatures) => {
          if (updatedApplicationAndFeatures) {
            if (updatedApplicationAndFeatures.wasUpdated) {
              console.debug(
                `[Tantalis sync] Application ${updatedApplicationAndFeatures.application._id} - data changed, database updated.`,
                updatedApplicationAndFeatures,
              )
            } else {
              console.debug(
                `[Tantalis sync] Application ${updatedApplicationAndFeatures.application._id} - data unchanged, no database write needed.`,
              )
            }
      ...
      .subscribe(
        ...
        (error) => {
          ...
          if (isAutoRefresh) {
            // Silent failure on auto-refresh: the page already shows the cached data.
            console.warn("Auto-refresh: couldn't sync with Tantalis", error)
          } else {
            console.log('error =', error)
        ...
        () => {
          this.isRefreshing = false
          if (!isAutoRefresh) {
            this.snackBarRef = this.snackBar.open('Application refreshed...', null, {
              duration: 2000,
            })
          }
        },
    ...
    this.dialogService
      .addDialog(
        ConfirmComponent,
        {
          ...
          message:
            'Publishing this application will make it visible to the public. Are you sure you want to proceed?',
          okOnly: false,
        },
        {
          backdropColor: 'rgba(0, 0, 0, 0.5)',
  ...
  private internalPublishApplication() {
    ...
    if (
      this.application.meta.currentPeriod &&
      !this.application.meta.currentPeriod.meta.isPublished
    ) {
      observables = observables.pipe(
        concat(this.commentPeriodService.publish(this.application.meta.currentPeriod)),
      )
    }
    ...
    if (this.application.meta.decision && !this.application.meta.decision.meta.isPublished) {
      observables = observables.pipe(
        concat(this.decisionService.publish(this.application.meta.decision)),
      )
    }
  ...
  public unPublishApplication() {
    ...
    if (
      ...
      observables = observables.pipe(
        concat(this.commentPeriodService.unPublish(this.application.meta.currentPeriod)),
      )
    }
    ...
    if (this.application.meta.decision && this.application.meta.decision.meta.isPublished) {
      observables = observables.pipe(
        concat(this.decisionService.unPublish(this.application.meta.decision)),
      )
    }
    ...
    observables.pipe(takeUntil(this.ngUnsubscribe)).subscribe(
      ...
      () => {
        // onCompleted
        this.snackBarRef = this.snackBar.open('Application unpublished...', null, {
          duration: 2000,
        })
        // reload all data
        this.applicationService
