...
export class ApiService {
  ...
  login(username: string, password: string): Observable<boolean> {
    return this.http
      .post<ILocalLoginResponse>(`${this.pathAPI}/login/token`, {
        username: username,
        password: password,
      })
      .pipe(
  ...
  public buildApplicationQueryParametersString(params: IApplicationQueryParamSet): string {
    ...
    if (params.status && params.status.value && params.status.value.length) {
      params.status.value.forEach(
        (status: string) => (queryString += `status[eq]=${encodeURIComponent(status)}&`),
      )
    }
    ...
    if (params.reason && params.reason.value && params.reason.value.length) {
      params.reason.value.forEach(
        (reason: string) =>
          (queryString += `reason[${params.reason.modifier}]=${encodeURIComponent(reason)}&`),
      )
    }
