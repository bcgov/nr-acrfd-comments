...
export class Application {
  ...
  featureCentroids: number[][] = [] // array of [lng, lat] for individual feature locations
  cl_file: number
  client: string
  ...
  constructor(obj?: any) {
    ...
    this.region =
      (obj && obj.businessUnit && ConstantUtils.getTextLong(CodeType.REGION, obj.businessUnit)) ||
      null
    ...
    // copy feature centroids
    if (obj && obj.featureCentroids) {
      obj.featureCentroids.forEach((centroid: number[]) => {
        this.featureCentroids.push(centroid)
      })
    }
