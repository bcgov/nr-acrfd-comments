...
export class ApplicationService {
  ...
  buildQueryParamSets(filters: IFiltersType, coordinates: string): IQueryParamSet[] {
    ...
    const basicQueryParams: IQueryParamSet = {
      ...
      purposes: {
        value:
          filters &&
          _.flatMap(filters.purposes, (purposeCode) =>
            ConstantUtils.getCode(CodeType.PURPOSE, purposeCode),
          ),
      },
      publishSince: {
        value: filters && filters.publishFrom ? filters.publishFrom.toISOString() : null,
      },
      publishUntil: {
        value: filters && filters.publishTo ? filters.publishTo.toISOString() : null,
      },
    ...
    const appStatusCodeGroups =
      (filters &&
        _.flatMap(filters.appStatuses, (statusCode) =>
          ConstantUtils.getCodeGroup(CodeType.STATUS, statusCode),
        )) ||
      []
  ...
  isAmendment(application: Application): boolean {
    return (
      ...
      (ConstantUtils.getCode(CodeType.REASON, application.reason) ===
        ReasonCodes.AMENDMENT_APPROVED.code ||
        ConstantUtils.getCode(CodeType.REASON, application.reason) ===
          ReasonCodes.AMENDMENT_NOT_APPROVED.code)
    )
  }
  ...
  isApplicationUnderReview(application: Application): boolean {
    return (
      ...
      ConstantUtils.getCode(CodeType.STATUS, application.status) ===
        StatusCodes.APPLICATION_UNDER_REVIEW.code
    )
  }
  ...
  isApplicationReviewComplete(application: Application): boolean {
    return (
      application &&
      ConstantUtils.getCode(CodeType.STATUS, application.status) ===
        StatusCodes.APPLICATION_REVIEW_COMPLETE.code
    )
  }
  ...
  isUnknown(application: Application): boolean {
    return (
      application &&
      ConstantUtils.getCode(CodeType.STATUS, application.status) === StatusCodes.UNKNOWN.code
    )
  }
  ...
  getStatusStringLong(application: Application): string {
    ...
    return (
      (application && ConstantUtils.getTextLong(CodeType.STATUS, application.status)) ||
      StatusCodes.UNKNOWN.text.long
    )
  }
}
