...
export class AppMapComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  public ngOnChanges(changes: SimpleChanges) {
    if (
      ...
      changes.applications.currentValue
    ) {
  ...
  private drawMap(deletedApps: Application[], addedApps: Application[]) {
    ...
    deletedApps.forEach((app) => {
      // remove all markers associated with this application (may be multiple if featureCentroids exist)
      for (let i = this.markerList.length - 1; i >= 0; i--) {
        if (this.markerList[i].dispositionId === app.tantalisID) {
          this.markerClusterGroup.removeLayer(this.markerList[i])
    ...
    addedApps.forEach((app) => {
      ...
      // Use individual feature centroids if available (multiple pins per application)
      // Otherwise fall back to the overall centroid (single pin per application)
      const centroids =
        app.featureCentroids && app.featureCentroids.length > 0
          ? app.featureCentroids
          : [app.centroid]
